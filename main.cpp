...
int main() {
    ...
    // Pregunta 1: Persona mas longeva
    PersonaCpp longeva = encontrarPersonaMasLongeva(personas);
    std::cout << "\nPersona mas longeva (Pais): " << longeva.nombreCompleto << " (" << calcularEdad(longeva.fechaNacimiento) << " anios)" << std::endl;
    auto longevaPorCiudad = encontrarMasLongevaPorCiudad(personas);
    imprimirResultadosPorCiudad("Persona mas longeva (por ciudad, top 5)", longevaPorCiudad);

    // Pregunta 2: Persona con mayor patrimonio
    PersonaCpp rica = encontrarPersonaMayorPatrimonio(personas);
    std::cout << "\nPersona con mayor patrimonio neto (Pais): " << rica.nombreCompleto << " ($" << rica.patrimonio - rica.deudas << ")" << std::endl;
    auto ricaPorCiudad = encontrarMasRicaPorCiudad(personas);
    imprimirResultadosPorCiudad("Persona mas rica (por ciudad, top 5)", ricaPorCiudad);
    auto ricaPorGrupo = encontrarMasRicaPorGrupo(personas);
    imprimirResultadosPorGrupo("Persona mas rica (por grupo)", ricaPorGrupo);

    // Pregunta 3: Conteo y validacion de declarantes
    ...
    std::cout << "\n[Paso 4: Comparaciones Tecnicas del Rendimiento]" << std::endl;
    
    // Comparacion Struct vs Class
    std::cout << "\n--- Comparacion Struct (C) vs Class (C++) ---" << std::endl;
    ...
    // Mostrar encabezado para la comparacion de rendimiento al procesar 5 millones de registros
    ...
    std::cout << "-> Tiempo de ejecucion: " << duracion_valor.count() << " segundos." << std::endl;
    
    std::cout << "Procesando " << N_ITERACIONES << " registros por REFERENCIA:" << std::endl;
    std::cout << "-> Tiempo de ejecucion: " << duracion_referencia.count() << " segundos." << std::endl;
